...
#p4-nsearch-wrap .srch-ico {
    ...
    top: calc(50% - 3px);          /* vertically center in the input, not the wrapper */
    transform: translateY(-50%);
    color: var(--text-muted); font-size: .62rem; pointer-events: none;
}
#p4-nsearch {
    width: 100%;
    background: rgba(255,255,255,.05);
...
function _patchFn(name, after, guardProp) {
    /* Retry until `name` is available on window, then wrap it */
    if (typeof window[name] !== 'function') { setTimeout(() => _patchFn(name, after, guardProp), 200); return; }
