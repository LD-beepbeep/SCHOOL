...
/* ================================================================
   FIX 5 — FORMULA PREVIEW  (never shows before typing)
   ================================================================ */
(function fixFormulaPreview() {

    /* The problem: patches3.js KaTeX hooks auto-render formula content
       when the modal opens. We need to intercept AFTER that render
       and reset the preview to blank if the modal was opened for a NEW formula. */

    function _clearPreview() {
        const preview = document.getElementById('formula-modal-preview');
        if (!preview) return;
        // Remove KaTeX output and replace with placeholder
        preview.innerHTML = '<span style="color:var(--text-muted);font-size:.78rem;font-style:italic;">Start typing to see a preview…</span>';
        preview.style.opacity = '0.55';
        preview.style.minHeight = '2rem';
        // Mark as cleared so input handler knows
        preview.dataset.cleared = '1';
    }

    function _attachPreviewInput(forceReset) {
        const preview  = document.getElementById('formula-modal-preview');
        const inp      = document.getElementById('formula-modal-formula');
        if (!inp) return;

        // If opening for a new formula (empty input), reset preview
        if (forceReset || !inp.value.trim()) {
            setTimeout(_clearPreview, 30);   // after patches3 KaTeX runs
            setTimeout(_clearPreview, 100);  // double-tap to defeat any async render
            setTimeout(_clearPreview, 300);
        }

        if (inp.dataset.p7Preview) return;  // already attached
        inp.dataset.p7Preview = '1';
        ...
        inp.addEventListener('input', () => {
            ...
            _t = setTimeout(() => {
                ...
                if (window.katex) {
                    try {
                        preview.innerHTML = window.katex.renderToString(val, {
                            throwOnError: false,
                            displayMode: true,
                        });
                    } catch { preview.textContent = val; }
                } else if (window.renderMathInElement) {
                    preview.textContent = val;
                    try { window.renderMathInElement(preview, {
    ...
    /* Patch openModal — run AFTER patches3 and patches6 have already patched it */
    function _patchOpenModal() {
        const orig = window.openModal;
        if (!orig || orig._p7FormPrev) { setTimeout(_patchOpenModal, 200); return; }
        window.openModal = function(id) {
            orig.apply(this, arguments);
            if (id !== 'modal-formula') return;
            // Determine if this is a NEW formula (null arg) or an EDIT
            // patches6/features.js calls formulaOpenModal(null) for new
            const editId = document.getElementById('formula-modal-edit-id')?.value;
            const isNew  = !editId;
            setTimeout(() => _attachPreviewInput(isNew), 50);
        };
        window.openModal._p7FormPrev = true;
    }
    setTimeout(_patchOpenModal, 400);

    /* Also intercept formulaOpenModal directly for robustness */
    function _patchFormulaOpenModal() {
        const orig = window.formulaOpenModal;
        if (!orig || orig._p7Fixed) { setTimeout(_patchFormulaOpenModal, 300); return; }
        window.formulaOpenModal = function(editId) {
            orig.apply(this, arguments);
            // After modal opens, clear preview if new formula
            if (!editId) {
                setTimeout(_clearPreview, 30);
                setTimeout(_clearPreview, 120);
                setTimeout(_clearPreview, 350);
            }
            setTimeout(() => _attachPreviewInput(!editId), 60);
        };
        window.formulaOpenModal._p7Fixed = true;
    }
    setTimeout(_patchFormulaOpenModal, 400);
